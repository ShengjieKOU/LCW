...
float time_lanechange = 1.5 ; // 经验换道时间
float time_reaction = 1.8 ; // 驾驶员反应时间
float distance_safe_heart = 3 ; // 紧急制动结束最小心理安全距离
float acceleration_max= 7 ; // 最大制动减速度
float acceleration_adjust=2 ; //跟车调整减速度


void dias::adas::CFunc_lcw::CarParam_set(OBSVector3d VehSpeed,float YawVelocity,OBSVector3d VehAcceleration){
     _VehSpeed = VehSpeed; //自车车速
     _YawVelocity = YawVelocity; //横摆角速度
     _VehAcceleration = VehAcceleration;//自车加速度
}

void dias::adas::CFunc_lcw::SelfLaneFront_SafeDist(TARGETS_INFO &_Targets) // 自车道前车
{
...
void dias::adas::CFunc_lcw::TargetLaneFront_SafeDist(TARGETS_INFO &_Targets) // 目标车道前车
{
    ...
    for (int ii=0 ; ii<targets_num ;ii++)
    {
        ...
        // std::cout<<"ego speed :"<< _VehSpeed._x << std::endl ;
        // std::cout<<"target speed:"<<_Targets._target_info[ii]._velocity._x << std::endl ; 
        // std::cout<<"ego acc :"<< _VehAcceleration._x << std::endl ;
        // std::cout<<"target acc:"<<_Targets._target_info[ii]._acceleration._x << std::endl ;
        // std::cout<< "current dist :"<< _Targets._target_info[ii]._dist <<"safe dist :"<< distance_safe_min[ii] << std::endl ;
...
void dias::adas::CFunc_lcw::TargetLaneRear_SafeDist(TARGETS_INFO &_Targets) // 目标车辆后车
{
...
int dias::adas::CFunc_lcw::Find_Risk_Target(TARGETS_INFO _Targets) // 返回最危险的目标id, 最危险车辆为纵向最近的目标
{
    int danger_car = 0 ;
    int targets_num=_Targets._target_info.size();
    if (targets_num==0) //没有危险目标
    {
        danger_car=-1 ;
    }
